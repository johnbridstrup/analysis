...
for key, sample in data.items():
    ...
    plt.plot(sample['t'][1:],sample['P_cv'][1:], label = '{}'.format(key))
    plt.legend()
    plt.title('Relative number deviation vs t')
plt.figure()
for key, sample in data.items():
    # plt.figure()
    # plt.plot(sample['t'][1:],sample['M_norm'][1:])
    ...
    plt.plot(sample['t'][1:],sample['L_cv'][1:], label = '{}'.format(key))
    plt.legend()
    plt.title('Relative length deviation vs t')

fig,axes = plt.subplots(2,2)
xidx=0
yidx=0
for key,sample in data.items():
    axes[yidx,xidx].plot(sample['t'],sample['L_dev'])
    axes[yidx,xidx].text(0.75, 0.75, 'N = {}'.format(key),
            verticalalignment='top', horizontalalignment='center',
            transform=axes[yidx,xidx].transAxes, fontsize=12)
    xidx = xidx+1
    if xidx % 2 == 0:
        xidx=0
        yidx=yidx+1
...
for key,sample in data.items():
    sample['reactions'].plot(x='t', y='sum')
    sample['reactions'].plot(x='t', y=['nucleation','addition','subtraction','coagulation','fragmentation'])
plt.show()
